...
class LLMComplectionError(Exception):
    pass
...
async def generate_llm_completion(
    ...
    try:
        ...
        raise LLMComplectionError(f"GigaChat API call failed: {exc}") from exc
    except Exception as exc:
        logger.error("Unexpected error during LLM generation: %s", exc, exc_info=True)
        raise LLMComplectionError(f"Unexpected LLM error: {exc}") from exc

    if not response.choices or not response.choices[0].message.content:
        logger.error("LLM return empty response or choices. Response: %s", response)
        raise LLMComplectionError("LLM returned empty response or response was blocked")
